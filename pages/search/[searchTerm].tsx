...
const Search = ({ videos }: { videos: Video[] }) => {
  ...
  return (
    <div className='w-full  '>
      <div className='flex gap-10 mb-10 border-b-2 border-gray-200 md:fixed z-50 bg-inherit w-full'>
            ...
            searchedAccounts.map((user: IUser, idx: number) => (
              <Link key={idx} href={`/profile/${user._id}`}>
                <div className=' flex gap-3 p-2 cursor-pointer font-semibold rounded border-b-2 border-gray-200'>
                  ...
                  <div>
                    <div>
                      <p className='flex gap-1 items-center text-lg font-bold text-secondary'>
                        {/* {user.userName} <GoVerified className='text-[#f51997]-400' /> */}
                        {user.userName} <FontAwesomeIcon icon={faUserShield} className='text-[#f51997]-400' />
                      </p>
